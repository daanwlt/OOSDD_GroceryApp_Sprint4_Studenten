...
- ProductService.Add(invalidProduct) → ArgumentException
```
...
#### **Test Case 3.2: Stock Management Edge Cases**
```csharp
// Test Scenario: Stock-related business logic
// Expected: Proper stock calculations
// Risk: Stock arithmetic errors

Test Cases:
- Product.IsInStock with zero stock → false
- Product.IsOutOfStock with positive stock → false
- Product.HasSufficientStock with negative amount → false
```
